...
WeightLoader::WeightLoader(const string& weights_dir) 
...
vector<float> WeightLoader::loadWeight(const string& filename, int expected_count) {
    string full_path = weights_dir_ + filename;
    ifstream file(full_path, ios::binary);
    
    if (!file.is_open()) {
        throw runtime_error("[ERROR] Could not open weight file: " + full_path);
    }
    ...
    if (expected_count > 0 && count != static_cast<size_t>(expected_count)) {
        throw runtime_error("[ERROR] Weight count mismatch in " + filename + ". Expected: " + to_string(expected_count) + ", Got: " + to_string(count));
    }
...
PatchEmbedWeights WeightLoader::loadPatchEmbedWeights(int stage) {
    ...
    int proj_weight_count = weights.embed_dim * weights.in_channels * weights.patch_size * weights.patch_size;
...
StageLayerNormWeights WeightLoader::loadStageLayerNormWeights(int stage) {
    ...
    string weight_file = "encoder_layer_norm_" + to_string(stage) + "_weight.bin";
    string bias_file = "encoder_layer_norm_" + to_string(stage) + "_bias.bin";
...
TransformerBlockWeights WeightLoader::loadTransformerBlockWeights(int stage, int block) {
    ...
    string prefix = "encoder_block_" + to_string(stage) + "_" + to_string(block) + "_";
    ...
    if (weights.sr_ratio > 1) {
        int sr_weight_count = weights.embed_dim * weights.embed_dim * 
                             weights.sr_ratio * weights.sr_ratio;
