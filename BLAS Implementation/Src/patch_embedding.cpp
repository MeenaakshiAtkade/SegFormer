...
PatchEmbedding::PatchEmbedding(int stage, int in_channels, int embed_dim,
...
void PatchEmbedding::loadWeights(const PatchEmbedWeights& w) {
    proj_weight_ = w.proj_weight;
    proj_bias_   = w.proj_bias;
    ln_weight_   = w.ln_weight;
    ln_bias_     = w.ln_bias;
}

vector<float> PatchEmbedding::conv2d_nchw(
    const vector<float>& input, int H, int W)
{
    ...
    vector<float> out(embed_dim_ * output_height_ * output_width_, 0.f);
                ...
                for (int ic = 0; ic < in_channels_; ++ic) {
                    for (int kh = 0; kh < patch_size_; ++kh) {
                        for (int kw = 0; kw < patch_size_; ++kw) {
                            ...
                            int w_idx =
                                oc * (in_channels_ * patch_size_ * patch_size_) +
                                ic * (patch_size_ * patch_size_) +
                                kh * patch_size_ + kw;

                            sum += static_cast<double>(input[in_idx]) * 
                                   static_cast<double>(proj_weight_[w_idx]);
                        }
                    }
                }

                int out_idx = oc * output_height_ * output_width_
                            + oh * output_width_ + ow;
...
vector<float> PatchEmbedding::layerNorm(
    const vector<float>& x)
{
    ...
    // Input: NCHW format [c][h][w]
    // Output: HWC token format [h][w][c]
    vector<float> out(H * W * C);
    ...
    for (int h = 0; h < H; ++h) {
        for (int w = 0; w < W; ++w) {
            ...
            double inv_std = 1.0 / sqrt(var + static_cast<double>(eps));

            // Normalize and write to HWC token format
            for (int c = 0; c < C; ++c) {
                int nchw_idx = c * H * W + h * W + w;
                ...
                out[hwc_idx] = static_cast<float>(
                    normalized * static_cast<double>(ln_weight_[c]) + 
                    static_cast<double>(ln_bias_[c]));
            }
        }
    }
...
vector<float> PatchEmbedding::forward(
    const vector<float>& input, int H, int W)
{
    // Conv2d: NCHW input → NCHW output
    auto conv_out = conv2d_nchw(input, H, W);
    
    // LayerNorm: NCHW input → HWC token output
