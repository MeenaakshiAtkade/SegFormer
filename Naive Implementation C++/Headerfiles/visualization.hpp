...
public:
    ...
    static std::vector<std::vector<std::vector<float>>> flattenedToHWC(const std::vector<float>& flat, int H, int W, int C) {
        
        std::vector<std::vector<std::vector<float>>> hwc(H, std::vector<std::vector<float>>(W, std::vector<float>(C)));
    ...
    static void saveFeatureMap(const std::vector<std::vector<std::vector<float>>>& hwc, const std::string& filename, int channel = 0) {
        ...
        std::cout << "Saved: " << filename << " (" << colored.rows << "x" << colored.cols << ")" << std::endl;
    }
    ...
    static void saveEncoderStage(const std::vector<std::vector<std::vector<float>>>& enc_hwc, const std::string& stage_name) {
        ...
        std::cout << "Saving " << stage_name << " (" << H << "x" << W << "x" << C << ")" << std::endl;
    ...
    static void saveDecoderStage(const std::vector<float>& flat, int H, int W, int C, const std::string& stage_name) {
        std::cout << "Saving decoder " << stage_name << " (" << H << "x" << W << "x" << C << ")" << std::endl;
    ...
    static cv::Mat predictionsToColored(const std::vector<std::vector<int>>& predictions) {
        ...
        // Cityscapes palette (19 classes) - RGB format
